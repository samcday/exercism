...
fn test_transcribes_cytosine_guanine() {
    assert_eq!(dna::RNA::new("G").unwrap(), dna::DNA::new("C").unwrap().to_rna());
}

#[test]
fn test_transcribes_guanine_cytosine() {
    assert_eq!(dna::RNA::new("C").unwrap(), dna::DNA::new("G").unwrap().to_rna());
}

#[test]
fn test_transcribes_adenine_uracil() {
    assert_eq!(dna::RNA::new("U").unwrap(), dna::DNA::new("A").unwrap().to_rna());
}

#[test]
fn test_transcribes_thymine_to_adenine() {
    assert_eq!(dna::RNA::new("A").unwrap(), dna::DNA::new("T").unwrap().to_rna());
}
